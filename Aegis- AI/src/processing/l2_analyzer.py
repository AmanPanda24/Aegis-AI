...
class L2Analyzer:
    def __init__(self, config=None):
        ...
        # Was a full mac_flood_window-length cooldown, which could mask a
        # second, independent flood starting shortly after the first
        # alert for the entire window. A stable "attacker" key isn't
        # available here (the attack's whole point is spraying random
        # source MACs, so there's nothing reliable to key a per-attacker
        # cooldown on) - shortening the cooldown instead of eliminating it
        # keeps sustained/overlapping floods visible without spamming.
        self.mac_flood_alert_cooldown = t.get("mac_flood_alert_cooldown_seconds", max(1, self.mac_flood_window // 2))
        ...
        # How many consecutive BPDUs claiming the SAME new (numerically
        # higher / non-superior) root id are required before we trust it
        # as a legitimate re-election - see _check_stp for why this
        # exists.
        self.stp_recovery_confirmations = t.get("stp_recovery_confirmations", 3)
        # ARP: trusted ip -> mac bindings we've observed
        self.arp_table = {}
        # MAC flood: rolling window of (timestamp, mac) seen
        self._mac_events = deque()
        # mac -> last-seen timestamp (was a plain `set` that only ever
        # grew - under a sustained MAC-flood/CDP-flood attack, exactly
        # the traffic this module exists to catch, that meant the
        # detector's own memory grew unbounded for the life of the
        # process: a slow, self-inflicted DoS against Aegis-AI itself.
        # Now periodically pruned - see _prune_stale_state.)
        self._known_macs = {}
        self._mac_flood_last_alert = 0
        # STP: baseline root bridge id, once learned
        self.stp_root_id = None
        self.stp_root_mac = None
        self._stp_candidate_root_id = None
        self._stp_candidate_count = 0
        self._stp_tc_events = deque()
        self._stp_tc_last_alert = 0
        ...
        # Retention window for per-mac state (_known_macs, _cdp_events,
        # _dtp_last_alert). A mac that's been silent this long is dropped
        # from tracking; if it reappears it's simply treated as "new"
        # again, which is harmless for these detectors. Bounds memory
        # under sustained attacks that spray many distinct source MACs.
        self.state_retention_seconds = t.get("state_retention_seconds", 3600)
        self._prune_interval_seconds = 60
        self._last_prune = 0.0

    def _prune_stale_state(self, now):
        if now - self._last_prune < self._prune_interval_seconds:
            return
        self._last_prune = now
        cutoff = now - self.state_retention_seconds

        stale_macs = [m for m, last_seen in self._known_macs.items() if last_seen < cutoff]
        for m in stale_macs:
            del self._known_macs[m]

        stale_cdp = [
            m for m, window in self._cdp_events.items()
            if not window or window[-1] < cutoff
        ]
        for m in stale_cdp:
            del self._cdp_events[m]

        stale_dtp = [m for m, last in self._dtp_last_alert.items() if last < cutoff]
        for m in stale_dtp:
            del self._dtp_last_alert[m]

    def analyze(self, event):
        etype = event.get("type")
        self._prune_stale_state(event.get("timestamp", time.time()))
        if etype == "ARP":
    ...
    def _check_mac_flood(self, event):
        ...
        if new_macs_in_window >= self.mac_flood_new_macs:
            if ts - self._mac_flood_last_alert < self.mac_flood_alert_cooldown:
                return None
    ...
    def _check_stp(self, event):
        ...
        if root_id:
            if self.stp_root_id is None:
                ...
                self._stp_candidate_root_id = None
                self._stp_candidate_count = 0
            elif root_id == self.stp_root_id:
                self._stp_candidate_root_id = None
                self._stp_candidate_count = 0
            elif root_id < self.stp_root_id:
                # Superior (lower) claim = classic hijack pattern; alert
                ...
                # root_id > self.stp_root_id: not the hijack pattern (an
                # attacker claims a *lower* id to win root election), but
                # previously this branch did nothing at all - meaning once
                # any root_id became the baseline (including an attacker's
                # forged one from a prior hijack alert above), a genuine
                # topology change electing a real, legitimately-higher-ID
                # root could never be relearned. The detector's baseline
                # stayed permanently stale after a single attack, unable
                # to recognize either the network healing or a *different*
                # later attack correctly.
                #
                # Require a few consecutive BPDUs claiming the same new id
                # before adopting it, so one stray/spoofed BPDU can't reset
                # tracking, but a sustained, real re-election is silently
                # relearned (no alert - this isn't attack behavior).
                if root_id == self._stp_candidate_root_id:
                    self._stp_candidate_count += 1
                else:
                    self._stp_candidate_root_id = root_id
                    self._stp_candidate_count = 1
                if self._stp_candidate_count >= self.stp_recovery_confirmations:
                    self.stp_root_id = root_id
                    self.stp_root_mac = event.get("src_mac")
                    self._stp_candidate_root_id = None
                    self._stp_candidate_count = 0
        ...
        if event.get("topology_change"):
            ...
            if (not alert and len(self._stp_tc_events) >= self.stp_tc_count
                    and ts - self._stp_tc_last_alert >= self.stp_tc_window):
                # Cooldown mirrors MAC_FLOODING's _mac_flood_last_alert gate
                # (and CDP_FLOOD's window.clear()): without it, every single
                # qualifying event after the threshold is crossed re-fires
                # its own alert - one attack burst of e.g. 8 topology-change
                # BPDUs was observed producing 4+ near-duplicate alerts
                # instead of one, drowning out genuinely distinct alerts
                # like STP_ROOT_HIJACK in the Attack Distribution chart.
                self._stp_tc_last_alert = ts
                return self._alert(
